...
IMPORTANT: Você deve atualizar para o pacote de serviços de gerenciamento mais recente antes de atualizar o software Element para a versão 12. Se estiver a atualizar o software Element para a versão 12,2, necessita de serviços de gestão 2.14.60 ou posterior para prosseguir.
...
IMPORTANT: Se você estiver atualizando um nó da série H610S para o elemento 12,2 e o nó estiver executando uma versão do elemento anterior a 11,8, será necessário executar etapas adicionais de atualização (<<Atualizando H610S nós de storage para o elemento 12,2 (fase 2),fase 2>>) para cada nó de storage. Se você estiver executando o elemento 11,8 ou posterior, as etapas adicionais de atualização (fase 2) não serão necessárias.
...
... Consulte <<Upgrading H610S storage nodes to Element 12.2 or later (phase 2)>> e conclua o processo para cada nó.
... Reautentique se necessário e selecione *put ​/Upgrades/
... Selecione *Experimente*.
... Insira o ID de atualização da etapa anterior em *upgradeId*.
...
Você pode usar o conjunto de ferramentas HealthTools para atualizar o software NetApp Element em um site escuro.
...
== Atualizando H610S nós de storage para o elemento 12,2 (fase 2)

Se você estiver atualizando um nó da série H610S para o elemento 12,2 e o nó estiver executando uma versão do elemento anterior a 11,8, o processo de atualização envolve duas fases.

A fase 1, que é realizada primeiro, segue os mesmos passos que a atualização padrão para o processo do elemento 12,2. Ele instala o Element Software e todas as atualizações de firmware do 5 de forma contínua no cluster, um nó de cada vez. Devido à carga útil do firmware, estima-se que o processo leve de aproximadamente 1,5 a 2 horas por nó H610S, incluindo um único ciclo de inicialização a frio no final da atualização para cada nó.
